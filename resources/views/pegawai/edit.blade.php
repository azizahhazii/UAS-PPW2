        ...
        <div class="mx-auto max-w-2xl">
            ...
            <form action="{{ route('pegawai.update') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center">
                            {{-- Value diganti 'male', tapi tetap cek jika data lama 'L' --}}
                            <input type="radio" name="gender" value="male" 
                                   {{ (old('gender', $data->gender) == 'male' || old('gender', $data->gender) == 'L') ? 'checked' : '' }}
                                   class="mr-2" required>
                            <span class="text-sm">Laki-laki</span>
                        </label>
                        <label class="flex items-center">
                            {{-- Value diganti 'female', tapi tetap cek jika data lama 'P' --}}
                            <input type="radio" name="gender" value="female" 
                                   {{ (old('gender', $data->gender) == 'female' || old('gender', $data->gender) == 'P') ? 'checked' : '' }}
                                   class="mr-2" required>
                ...
                <div class="flex gap-3 pt-4">
                    <button type="submit" 
                            class="rounded-md bg-blue-600 px-6 py-2 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        Update
                    </button>
